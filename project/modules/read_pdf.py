...
def check_PDF(text: str) -> result_class:
        ...
        if 'fyzicka osoba' in [canon_word for _, (canon_word, _) in matches]:
            message = f'WARNING: alias for fyzicka osoba was mentioned'
            messages.append(message)
        if 'adress' in [canon_word for _, (canon_word, _) in matches]:
            message = f'WARNING: alias for bydlisko was mentioned'
            messages.append(message)
...
if __name__ == '__main__':
    ...
    project_folder = project_root.resolve().parent.parent.parent
